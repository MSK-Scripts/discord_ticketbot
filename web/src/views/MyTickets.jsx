...
export default function MyTickets({ me, ticketId, onOpen, onClose }) {
  ...
  return (
    <>
      <div className="mb-5"><h1 className="font-display text-xl font-bold">My tickets</h1></div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Every ticket you opened. You can reply to open ones right here — your reply is
            posted in the Discord channel under your name.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!items ? <Empty>Loading…</Empty> : items.length === 0 ? (
            <Empty>You have not opened any tickets yet.</Empty>
          ) : (
            <Table>
              <TableHeader>
                <TableRow><TableHead>#</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead><TableHead>Priority</TableHead><TableHead>Created</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {items.map(t => (
